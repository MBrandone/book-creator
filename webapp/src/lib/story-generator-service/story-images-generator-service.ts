...
export class StoryImagesGeneratorService {
  ...
  async generate(story: Story): Promise<void> {
    try {
      ...
      console.log(`[${story.id}] 📸 Converting ${referenceImageKeys.length} reference photos to data URIs...`);
      ...
      console.log(`[${story.id}] ✅ ${referenceImages.length} reference photos converted successfully`);
      console.log(`[${story.id}] 🖼️  Starting image generation for ${story.scenes.length} scenes...`);
      ...
      for (const scene of story.scenes) {
        try {
          ...
          await new Promise(resolve => setTimeout(resolve, this.SECONDS_TO_WAIT * 1000));

        } catch (imageError) {
          console.error(`[${story.id}] ❌ Failed to generate image for scene ${scene.sceneNumber}:`, imageError);
          throw new Error(`Failed to generate image for scene ${scene.sceneNumber}: ${imageError instanceof Error ? imageError.message : 'Unknown error'}`);
        }
      }
      ...
      console.log(`[${story.id}] ✅ Images generated successfully for ${story.scenes.length} scenes`);
