...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    const idValidationResult = z.string().uuid('L\'ID de l\'histoire doit être un UUID valide').safeParse(storyId);
    const sceneIdValidationResult = z.string().uuid('L\'ID de la scène doit être un UUID valide').safeParse(sceneId);
    ...
    const commandHandler = new UpdateSceneDescriptionCommandHandler(storyRepository);

    await commandHandler.execute(storyId, sceneId, validationResult.data.description);
    ...
    if (error instanceof SceneDescriptionTooShortError || error instanceof SceneDescriptionTooLongError) {
      return NextResponse.json(
        {
          error: 'Validation échouée',
          details: [{
