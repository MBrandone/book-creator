...
export async function up(db: Kysely<any>): Promise<void> {
	...
	await db.schema
		...
		.addCheckConstraint(
			"scenes_scene_number_check",
			sql`scene_number BETWEEN 1 AND 4`
		)
		.addUniqueConstraint("scenes_story_scene_unique", [
			"story_id",
			"scene_number",
		])
		.execute();
	...
	await db.schema
		.createIndex("idx_characters_story_id")
		.on("characters")
		.column("story_id")
		.execute();
	await db.schema
		.createIndex("idx_scenes_story_id")
		.on("scenes")
		.column("story_id")
		.execute();
	await db.schema
		.createIndex("idx_scenes_scene_number")
		.on("scenes")
		.columns(["story_id", "scene_number"])
		.execute();
	await db.schema
		.createIndex("idx_stories_status")
		.on("stories")
		.column("status")
		.execute();
	await db.schema
		.createIndex("idx_stories_created_at")
		.on("stories")
		.column("created_at")
		.execute();
}
