...
const sceneTypeSchema = z.string()
  .transform((val) => StoryScenesDescriptionGeneratorValidator.normalizeSceneType(val))
  .refine((val): val is SceneType => {
    const validSceneTypes: SceneType[] = ['introduction', 'conflict', 'action', 'resolution'];
    return validSceneTypes.includes(val as SceneType);
  }, {
    message: "scene_type must be one of: introduction, conflict, action, resolution"
  });

const aiSceneResponseSchema = z.object({
...
const aiStoryResponseSchema = z.object({
  scenes: z.array(aiSceneResponseSchema)
