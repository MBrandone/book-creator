...
export function CharacterPhotoUpload({
	...
	const uploadMutation = useMutation({
		mutationFn: async (file: File) => {
			const { uploadUrl, storageKey, storageBucket } = await getUploadUrl(
				file.type
			);
	...
	return (
		<div className="space-y-2">
				...
				{!previewUrl ? (
					<div className="flex flex-col items-center justify-center py-6">
						...
						<p className="text-sm text-muted-foreground mb-3 text-center">
							Ajoutez une photo de référence pour personnaliser l'apparence du
							personnage
						</p>
						<Button
							type="button"
			...
			<p className="text-xs text-muted-foreground">
				Cette photo sera utilisée pour générer des images du personnage qui lui
				ressemblent
			</p>
		</div>
	);
